...
def supabase_client():
    """Singleton. Imported lazily so missing env vars surface as a
    per-job error in the response rather than a boot-time crash that
    only shows up in RunPod's worker logs."""
